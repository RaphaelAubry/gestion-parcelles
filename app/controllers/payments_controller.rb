class PaymentsController < ApplicationController
        ...
        render json: {
          ...
          data: @payments.map do |p|
                  [ p.payment_date,
                    p.amount,
                    p.invoice.number,
                    "<a href='/payments/#{p.id}/edit'>modifier</a>
                     <a href='/payments/#{p.id}' data-turbo-method='delete'>supprimer</a>
                    "
                  ]
                end
          }
  ...
  def create
    ...
    if @payment.save
      redirect_to payments_path, notice: "Paiement de #{sprintf("%.2f", @payment.amount)} enregistré"
    else
      @invoices = scope_invoices
  ...
  def update
    if @payment.update(payment_params)
      redirect_to payments_path, notice: "Paiement de #{sprintf("%.2f", @payment.amount)} modifié"
    else
      @invoices = scope_invoices
  ...
  def destroy
    ...
    redirect_to payments_path, notice: "Paiement supprimé."
  end
